...
import { isUndefined } from 'handlebars-utils';
...
export function bytes(number, precision, options) {
...
export function toAbbr(number, precision) {
  if (isNaN(number)) {
    number = 0;
  }
...
export function toExponential(number, digits) {
  if (isNaN(number)) {
    number = 0;
  }
