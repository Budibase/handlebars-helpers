...
import { number as _number } from '../index.mjs';
_number({handlebars: hbs});

describe('number', function() {
  describe('bytes', function() {
    it('should format a number', function() {
      equal(hbs.compile('{{bytes num}}')({num: 13661855}), '13.66 MB');
      equal(hbs.compile('{{bytes num}}')({num: 825399}), '825.4 kB');
      equal(hbs.compile('{{bytes num}}')({num: 1396}), '1.4 kB');
      equal(hbs.compile('{{bytes num}}')({num: 0}), '0 B');
      equal(hbs.compile('{{bytes num}}')({num: 1}), '1 B');
      equal(hbs.compile('{{bytes num}}')({num: 2}), '2 B');
    });

    it('should return "0 B" when an invalid value is passed', function() {
      equal(hbs.compile('{{bytes num}}')({num: {}}), '0 B');
    });

    it('should return string length when a string is passed', function() {
      equal(hbs.compile('{{bytes num}}')({num: 'foo'}), '3 B');
      equal(hbs.compile('{{bytes num}}')({num: 'foobar'}), '6 B');
    });
  });
  ...
  describe('phoneNumber', function() {
    it('Format a phone number.', function() {
      ...
      equal(fn({value: '8005551212'}), '(800) 555-1212');
    });
  });
  ...
  describe('toPrecision', function() {
    it('Returns the number in fixed-point or exponential notation rounded to n significant digits.', function() {
      ...
      equal(fn({value: 555.322 }), '6e+2');
    });
    it('should return the value rounded exactly n digits after the decimal place.', function() {
      var fn = hbs.compile('{{toPrecision value 4}}');
      equal(fn({value: 555.322 }), '555.3');
    });
  });
  ...
  describe('addCommas', function() {
    it('should add commas to a number.', function() {
      ...
      equal(fn({value: 2222222 }), '2,222,222');
    });
  });
  ...
  describe('toAbbr', function() {
    it('should abbreviate the given number.', function() {
      ...
      equal(fn({number: 123456789 }), '123.46m');
    });

    it('should abbreviate a number with to the given decimal.', function() {
      var fn = hbs.compile('{{toAbbr number 3}}');
      equal(fn({number: 123456789 }), '123.457m');
    });

    it('should round up to the next increment', function() {
      var fn = hbs.compile('{{toAbbr number}}');
      equal(fn({number: 999 }), '1k');
    });

    it('should abbreviate a number based on a number and include decimal.', function() {
      equal(hbs.compile('{{toAbbr number 0}}')({number: 9999999 }), '10m');
      equal(hbs.compile('{{toAbbr number}}')({number: 1000000000 }), '1b');
      equal(hbs.compile('{{toAbbr number}}')({number: 1000000000000 }), '1t');
      equal(hbs.compile('{{toAbbr number}}')({number: 1000000000000000 }), '1q');
      equal(hbs.compile('{{toAbbr number}}')({number: 99393999393 }), '99.39b');
    });
  });
});
