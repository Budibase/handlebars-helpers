...
describe('assemble', function() {
  describe('absolute', function() {
    it('should create an absolute file path', function() {
      equal(hbs.compile('{{absolute "a/b/c/package.json"}}')(), path.resolve('a/b/c/package.json'));
      equal(hbs.compile('{{absolute "a/b/c/docs/toc.md"}}')(), path.resolve('a/b/c/docs/toc.md'));
    });

    it('should use the cwd on locals', function() {
      equal(hbs.compile('{{absolute "a/b/c/package.json"}}')({cwd: os.homedir()}), path.resolve(os.homedir(), 'a/b/c/package.json'));
      equal(hbs.compile('{{absolute "a/b/c/docs/toc.md"}}')({cwd: gm}), path.resolve(gm, 'a/b/c/docs/toc.md'));
    });
  });
  ...
  describe('relative', function() {
    ...
    it('should return the relative path from file A to folder B', function() {
      ...
      equal(fn(), path.join('..', '..', 'assets'));
    });
    it('should use the cwd passed on options', function() {
      var fn = hbs.compile('{{relative "examples/result/md/path.md" "examples/assets"}}');
      equal(fn({cwd: gm}), path.join('..', '..', 'assets'));
    });
  });
